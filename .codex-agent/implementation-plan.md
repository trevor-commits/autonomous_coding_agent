...
- Ось уникальности: runtime-owned correctness instead of AI-owned workflow
  theater
- Сигнал доверия: one writer, bounded queue legality, deterministic verification,
  and repo-visible closeout
- Что нельзя делать по шаблону: pretend this is a generic AI swarm, generic
  dashboard, or broad platform before one real repo is proven

## Выбранный вариант плана

- `Оптимально` (recommended, pending approval)
- Потому что он keeps the scope honest: it does not jump to Phase 4/5, but it
...
- If you approve `Оптимально` or `С запасом`, name the first implementation repo
  to onboard next.
