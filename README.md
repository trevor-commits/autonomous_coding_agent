...
The settled architecture keeps this tested deterministic supervisor and adds a stateless autonomous-partner policy above it. The existing global governor owns durable identity, goals, approved observations, approvals, learning promotion, heartbeat, and queue state. ACA ranks evidence-backed useful/repair/research/care/creative candidates, emits one typed decision per wake-mode intent, and executes only immutable authorized run contracts. Codex remains the sole writer, Playwright the sole browser owner, and deterministic evidence the only path to `COMPLETE` + `READY`.

For partner envelopes, the Codex builder is edit-only: it may apply scoped patches and use bounded filename discovery, but it cannot execute repo code or repo-contract checks. The supervisor runs deterministic checks afterward with a scrubbed environment, no inherited stdin, denied network, read access limited to the worktree/system runtimes, and writes limited to the approved run paths plus supervisor-owned temp state. Partner envelopes reject UI acceptance and never enter app/browser phases. Nested `.env*`, `.git`, and `.agent` residue is denied before effects and reconciled independently of Git after every builder or verifier phase.
