...
- Historical rounds 4 and 11 were clean only at their frozen tips. Round 17 repaired bounded output, timeout detach/reparent, descriptor-safe durable state, and lease-retaining rollback; round 18 repaired only a `blocked`/`denied` narration oracle. Fresh round 19 then proved that normal-success cleanup could leave a detached child alive after the final sandbox evidence boundary. The first ancestry-only repair failed immediate-detach stress and was strengthened with gated spawn, a continuous process-tree lease, inherited per-run lease discovery, and STOP-before-TERM/KILL cleanup on every exit path. Focused and concurrent stress evidence is green; full exact-tip repetition, current-main reconciliation, remote checks, and a fresh literal `REVIEW-CLEAN` remain mandatory. The generic guard-fixture comment remains rejected because production emits `allowed_commands: []` and the live probe proves repo checks are denied. Evidence: `records/audits/2026-07-13-autonomous-partner-independent-review.md`.
