...
LOCAL_VERIFY
  ...
  → if pass: continue toward FINAL_GATE or the next required phase
...
Failure fingerprinting is supervisor-owned rather than a strategy action in the smallest v1. The strategy can react to repeated fingerprints through prompts and terminal proposals, but it does not write the fingerprint record directly.
...
Codex edits files but does not own commit authority in v1. If run-level markers or landings are emitted later, the supervisor owns them. This keeps partial broken work from being mistaken for a final outcome.
...
Every run produces two outputs: a machine-readable JSON report (`run_id`, `claim_id`, `run_trace_id`, `run_state`, `readiness_verdict`, `phases_completed`, `commands_run`, `failures`, `changed_files`, `artifact_manifest`, `unresolved_blockers`, `queue_entry_reason`, `queue_exit_reason`, with optional `checkpoint_refs`) and a human-readable markdown summary (what changed, what passed, what failed, what's unresolved, what to do next).
...
Human writes run contract
  ...
  → For each milestone:
      ...
      → If pass: Supervisor advances to the next required phase
