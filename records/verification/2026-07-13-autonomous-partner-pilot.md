...
Exact remote checks and a fresh literal `REVIEW-CLEAN` remain the release gate. On non-macOS platforms the environment/ancestry fallback is weaker than the immutable macOS tag; Stage 0 deployment remains pinned to the verified macOS host rather than treating that fallback as equivalent containment.
...
Provenance: executor=codex:gpt-5.6-sol:high; audit=L4-pending; scripts=ACA-unittest,live-builder-containment,containment-stress,OpenSpec,project-memory,global-verify; escalations=0; routing=T3; notes=round-21 exact implementation tip passes deterministic/live/stress gates; record push, remote checks, and fresh review pending
