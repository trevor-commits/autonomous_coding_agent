...
- Historical rounds 4 and 11 were clean only at their frozen tips. The latest independent challenge then found remaining command-indirection, model-context, timeout-descendant, storage-link, actual-clock, publication-order, and production-override gaps. ACA `7b69f03` and global `3d27a32` repair them and the post-repair timing failures: absolute denials cover interpreter/package/Git-helper and direct outward-action routes, deterministic execution disables Git hooks/fsmonitor plus Apple Events, captured output is bounded, `setsid()` descendants are stopped/killed/reaped, supervisor storage rejects symlink/hardlink leaves, app-launch failures preserve the actual exit, loaded timeout fixtures use explicit no-live-PID proof, the idea model runs from an auth-only empty runtime, effect expiry uses actual UTC, and ready publication cannot precede its decision. ACA passes 211 tests both with the live Codex probe skipped and enabled; strict OpenSpec is 4/4; global candidate/bridge/executor/snapshot/reconcile are 8/15/19/4/6; a real production-shape candidate call succeeded; three simultaneous platform-probe suites and the complete global verifier are green. Fresh exact-tip independent review remains mandatory. Evidence: `records/audits/2026-07-13-autonomous-partner-independent-review.md`.
...
- Remaining release gates: fresh exact-tip `REVIEW-CLEAN`, origin/main landing, and observe-only immutable deployment proof. The empty approved-observation feed means this is a hardened substrate; one bounded low-sensitivity observation adapter and a graduated-agency design remain post-deployment product slices. That later design should default-allow reversible private/local work and reserve hard gates for concrete destructive, irreversible, credentialed, financial, privacy-expanding, security-sensitive, or outward-facing effects. Recurring execute remains off.
