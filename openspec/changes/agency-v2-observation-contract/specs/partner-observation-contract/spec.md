...
Every wake-v2 observation SHALL validate against the copied `partner-observation-v2.schema.json`. Unknown fields, missing fields, invalid collector names, invalid namespaces, observation-level opportunity scores, or non-low sensitivity MUST reject the complete wake.
