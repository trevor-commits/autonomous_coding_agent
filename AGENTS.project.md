...
- **Codex may:** create a Linear issue if none exists, append `todo.md` `Work Record Log` and `Completed` with the landing reference, update `todo.md` `Linear Issue Ledger` when a live issue is created or its repo-side home changes, post a completion comment on the Linear issue, and commit and push on the current branch. Completion evidence must stay issue-scoped: `Completed`, `led to:`, and the Linear completion comment cite only the commit(s) or artifacts whose primary outcome belongs to that issue. Later audit-only or correction commits for a different issue are logged on the correcting issue instead of being appended to the earlier issue's closeout. `Pre-commit gates:` `Continuity Check` (Work Record entry exists and Self-audit is non-hollow), `Ripple Check` (every affected live doc was checked and any drift was updated in the same commit, then attested in Self-audit), and `Linear-coverage` (every surfaced follow-up is filed as a `GIL-N` issue or dispositioned in the log entry, and every live issue has a ledger entry with provenance).
